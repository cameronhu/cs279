...
def mean(X):
  ...
  array_size = 6
  array_size_squared = array_size **2
  initial_tuple = (array_size, array_size)
  R = np.empty(initial_tuple)
  R.fill(1 / array_size_squared)
...
def hp(X):
  ...
  initial_tuple = (size, size)
  R = np.empty(initial_tuple)
  R.fill(-c)
  R[mid, mid] = (size_squared - 1) * c
  return ndimage.convolve(X, R)
  # END YOUR CODE HERE
